...
function calculateRankingDamage(data) {
  // ペットパッシブのデバッグ用
  const isPetData = data.length > 0 && data[0].category && !['初級', '中級', '上級'].includes(data[0].category);
  if (isPetData) {
    console.log(`=== ペットパッシブランキング計算開始 (${data.length}件) ===`);
  }
  ...
  for (const item of data) {
    ...
    if (item.status_value1) {
      ...
      // statusType未対応で0が返ってきた場合の警告
      if (dmg1 === 0 && item.status_value1 !== 0 && isPetData) {
        const typeName = StatusTypeMap[item.status_type1] || `Unknown(${item.status_type1})`;
        console.warn(`未対応statusType: ${item.status_type1} (${typeName}) - ${item.name}`);
      }
    }
    ...
    if (item.status_type2 > 0 && item.status_value2) {
      ...
      // statusType未対応で0が返ってきた場合の警告
      if (dmg2 === 0 && item.status_value2 !== 0 && isPetData) {
        const typeName = StatusTypeMap[item.status_type2] || `Unknown(${item.status_type2})`;
        console.warn(`未対応statusType: ${item.status_type2} (${typeName}) - ${item.name}`);
      }
    }
...
const calculatePetPassiveRankings = () => {
  console.log('=== calculatePetPassiveRankings 開始 ===');
  console.log('ペットパッシブ初期化データ:', ペットパッシブ初期化データ);
  const result = createRankingCalculator(ペットパッシブ初期化データ)();
  console.log('=== calculatePetPassiveRankings 終了 ===');
