...
class AwakenSkillTree {
  ...
  init() {
    ...
    if (typeof awakenData !== 'undefined') {
      ...
      // デバッグ：スロット1の座標表示
      console.log(`=== Awaken${this.page} Debug ===`);
      console.log('coordsData slot count:', Object.keys(coordsData).length);
      console.log('Slot 1 coords:', coordsData[1]);
      console.log('pageData nodes count:', pageData.nodes.length);
      ...
      console.log(`Awaken${this.page} - Max X: ${maxX}, Max Y: ${maxY}, Canvas: ${this.canvas.width}x${this.canvas.height}`);
  ...
  activateNode(node) {
    ...
    if (node.limits && node.limits.length > 0) {
      ...
      if (hasLimit) {
        console.log(`Node ${node.slot} activation failed: exclusive node active`);
        return;
      }
    }
    ...
    if (path) {
      console.log(`Auto-activating path to ${node.slot}: ${path.join(' -> ')}`);
      path.forEach(slot => this.activeNodes.add(slot));
    } else {
      console.log(`No valid path found to node ${node.slot}`);
    }
  }
  ...
  findBestPath(targetNode) {
    ...
    if (targetDistance === null) {
      console.log(`No path found to node ${targetNode.slot}`);
      return null;
    }
    
    console.log(`Shortest distance to node ${targetNode.slot}: ${targetDistance}`);
    ...
    console.log(`Found ${allPaths.length} paths with distance ${targetDistance}`);
    ...
    if (bestPath) {
      console.log(`Best path: ${bestPath.join(' -> ')}, score: ${bestScore.toFixed(2)}`);
    }
  ...
  deactivateNode(node) {
    // スロット1（スタート）はOFF禁止
    if (node.slot === 1) {
      console.log(`Node ${node.slot} is start node, cannot deactivate`);
      return;
    }
  ...
  removeUnreachableNodes() {
    ...
    if (toRemove.length > 0) {
      console.log(`Removed unreachable nodes: ${toRemove.join(', ')}`);
    }
  }
  ...
  showTooltip(x, y, clientX, clientY, node) {
    ...
    console.log('マウス座標 (clientX, clientY):', clientX, clientY);
    console.log('親要素 rect:', 'left=' + parentRect.left, 'top=' + parentRect.top, 'width=' + parentRect.width, 'height=' + parentRect.height);
    console.log('親要素基準座標 (relativeX, relativeY):', relativeX.toFixed(2), relativeY.toFixed(2));
    console.log('ツールチップサイズ:', tooltipWidth, tooltipHeight);
    console.log('ツールチップ位置 (left, top):', tooltip.style.left, tooltip.style.top);
  }
  ...
  autoOptimize() {
    ...
    if (maxPoints <= 0) {
      console.log('No points to allocate');
      return;
    }
    ...
    console.log(`Auto-optimize: ${maxPoints} points to allocate`);
    ...
    while (remainingPoints > 0) {
      ...
      if (!bestPath || bestPath.length === 0) {
        console.log('No more valid paths found');
        break;
      }
      
      // 最高効率のパスを追加
      console.log(`Best path efficiency: ${bestEfficiency.toFixed(2)}, adding ${bestPath.length} nodes: ${bestPath.join(',')}`);
      bestPath.forEach(slot => newActiveNodes.add(slot));
      remainingPoints -= bestPath.length;
    }
    
    console.log(`Auto-optimize complete: used ${maxPoints - remainingPoints} points`);
