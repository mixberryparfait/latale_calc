...
function calculateBaseDamage(stats, enemyStats, enemyType) {
...
const calculateDamages = (stats, enemyStats, enemyType) => {
  const baseDamage = calculateBaseDamage(stats, enemyStats, enemyType);
...
function calculateResults(enemyStats, enemyType) {
  ...
  const baseResults = calculateDamages(stats, enemyStats, enemyType);
  ...
  const damagePerStatPoint = key => {
    ...
    const newDamageResult = calculateDamages(tmp, enemyStats, enemyType);
...
// ダメージ換算（statusTypeは文字列、enemyTypeは '一般' または 'ボス'）
function calculateDamageIncrease(statusType, value, enemyType) {
  statusType = getActualKey(statusType);
  const damagePerStatKey = `damagePerStat_${enemyType}`;
  const damagePerStat = results.value[damagePerStatKey];
...
function calculateRankingDamage(data) {
  ...
  for (const item of data) {
    // 一般の絶対増加ダメージ
    let totalDamageIncreaseGeneral = 0;
    if (item.status_value1) {
      totalDamageIncreaseGeneral += calculateDamageIncrease(item.status_type1, item.status_value1, '一般');
    }
    if (item.status_type2 && item.status_value2) {
      totalDamageIncreaseGeneral += calculateDamageIncrease(item.status_type2, item.status_value2, '一般');
    }
    
    // ボスの絶対増加ダメージ
    let totalDamageIncreaseBoss = 0;
    if (item.status_value1) {
      totalDamageIncreaseBoss += calculateDamageIncrease(item.status_type1, item.status_value1, 'ボス');
    }
    if (item.status_type2 && item.status_value2) {
      totalDamageIncreaseBoss += calculateDamageIncrease(item.status_type2, item.status_value2, 'ボス');
    }
    
    // 相対値（増加率）の計算
    // 増加率 = 増加ダメージ / 元ダメージ
    const increaseRateGeneral = totalDamageIncreaseGeneral / baseDamageGeneral;
    const increaseRateBoss = totalDamageIncreaseBoss / baseDamageBoss;
    
    // 加重平均の増加率
    const weightedIncreaseRate = increaseRateGeneral * generalImportance + increaseRateBoss * bossImportance;
    
    // 増加率を平均ダメージに掛けて実際のダメージ値にする
    const actualDamageIncrease = weightedIncreaseRate * averageDamage;
...
const updateEnemyValues = (enemyType) => {
  ...
  const 敵名 = enemyType === '一般' ? 敵名_一般.value : 敵名_ボス.value;
  const 難易度 = enemyType === '一般' ? 難易度_一般.value : 難易度_ボス.value;
  const 敵ステータス = enemyType === '一般' ? 敵ステータス_一般 : 敵ステータス_ボス;
  
  const selectedEnemy = dungeonData.enemies.find(e => e.name === 敵名);
  if (!selectedEnemy) return;

  敵ステータス.value.防御 = selectedEnemy.baseStats.defense;
  敵ステータス.value.幸運 = selectedEnemy.baseStats.luck;

  if (難易度) {
    const difficultyData = selectedEnemy.difficultyStats[難易度];
    if (difficultyData) {
      敵ステータス.value.ダメ減 = difficultyData.dmgReduce;
      敵ステータス.value.ダメ減_乗算 = difficultyData.dmgReduceRatio;
      敵ステータス.value.クリ減 = difficultyData.critReduce;
    }
  }
};
...
const onDungeonChange = (enemyType) => {
  if (enemyType === '一般') {
    敵名_一般.value = '';
  } else {
    敵名_ボス.value = '';
  ...
  if (dungeonData) {
    ...
    if (firstEnemy && firstEnemy.difficultyStats) {
      ...
      if (difficulties.length > 0) {
        if (enemyType === '一般') {
          難易度_一般.value = difficulties[0];
        } else {
          難易度_ボス.value = difficulties[0];
        }
      }
    }
  }
  
  if (enemyType === '一般' && !難易度_一般.value) {
    難易度_一般.value = '';
  } else if (enemyType === 'ボス' && !難易度_ボス.value) {
    難易度_ボス.value = '';
  }
};

const onDifficultyChange = (enemyType) => {
  updateEnemyValues(enemyType);
};

const onEnemyChange = (enemyType) => {
  updateEnemyValues(enemyType);
};
...
const 加重平均ダメージ = computed(() => {
  const 一般結果 = 一般ダメージ.value || calculateResults(敵ステータス_一般.value, '一般');
  const ボス結果 = ボスダメージ.value || calculateResults(敵ステータス_ボス.value, 'ボス');
  
  const 一般重要度 = (100 - ボス重要度.value) / 100;
  const ボス重要度率 = ボス重要度.value / 100;
  
  return {
    damage: 一般結果.damage * 一般重要度 + ボス結果.damage * ボス重要度率,
    dps: 一般結果.dps * 一般重要度 + ボス結果.dps * ボス重要度率,
    baseDamage: 一般結果.baseDamage * 一般重要度 + ボス結果.baseDamage * ボス重要度率,
    criRate: 一般結果.criRate * 一般重要度 + ボス結果.criRate * ボス重要度率,
    criDamage: 一般結果.criDamage * 一般重要度 + ボス結果.criDamage * ボス重要度率
  };
});
...
const results = computed(() => {
  ...
  return {
    ...
    // ランキング計算用に両方のdamagePerStatを保持
    ダメージ_一般: 一般ダメージ.value.damage,
    ダメージ_ボス: ボスダメージ.value.damage,
    damagePerStat_一般: 一般ダメージ.value.damagePerStat,
    damagePerStat_ボス: ボスダメージ.value.damagePerStat
  };
});
...
const app = createApp({
  setup() {
    ...
    const 選択可能難易度_一般 = computed(() => {
      const dungeonData = get選択中ダンジョン敵データ('一般');
      if (!dungeonData) return [];
    ...
    const 選択可能難易度_ボス = computed(() => {
      const dungeonData = get選択中ダンジョン敵データ('ボス');
      if (!dungeonData) return [];
    ...
    const 選択可能敵_一般 = computed(() => {
      const dungeonData = get選択中ダンジョン敵データ('一般');
      if (!dungeonData) return [];
      
      return dungeonData.enemies
        .filter(enemy => enemy.difficultyStats && enemy.difficultyStats[難易度_一般.value])
        .map(enemy => enemy.name);
    });

    const 選択可能敵_ボス = computed(() => {
      const dungeonData = get選択中ダンジョン敵データ('ボス');
      if (!dungeonData) return [];
      
      return dungeonData.enemies
        .filter(enemy => enemy.difficultyStats && enemy.difficultyStats[難易度_ボス.value])
        .map(enemy => enemy.name);
    });
