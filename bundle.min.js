...
/*　マスターjson
{
  ...
  status_value_slv1: 初期値1（必須）,
  status_value1: レベル毎増分1（オプション）,
  status_type2: ステータスタイプ2（オプション）,
  status_value_slv2: 初期値2（オプション）,
  status_value2: レベル毎増分2（オプション）,
}
...
function initializeRankingData(skills) {
  ...
  for (const skill of skills) {
    ...
    for (let level = 1; level <= maxIterations; level++) {
      ...
      // このレベルでの実際の値を計算
      const value1 = skill.status_value_slv1 + (skill.status_value1 || 0) * (level - 1);
      const value2 = skill.status_type2 > 0 ? 
        (skill.status_value_slv2 || 0) + (skill.status_value2 || 0) * (level - 1) : 0;
