...
const 装備ランキング種別フィルタ = ref({
  強化: true,
  エンチャ: true,
  覚醒: true
});
const 装備ランキング未装備値 = '未装備';
const 装備ランキング選択装備 = ref(装備ランキング未装備値);
const 装備ランキング装備候補 = ref([]);
const 装備ランキング装備候補カテゴリ別 = ref({});
...
const loadFromStorage = () => {
  try {
    ...
    if (saved) {
      ...
      if (typeof data.装備ランキング選択装備 === 'string' && data.装備ランキング選択装備) {
        装備ランキング選択装備.value = data.装備ランキング選択装備;
      } else {
        装備ランキング選択装備.value = 装備ランキング未装備値;
      }
...
function calculateResults(enemyStats, enemyType) {
  ...
  const damagePerStatPoint = key => {
    ...
    if (key === '体力') {
      ...
      const HP_乗算 = stats.HP_乗算;
      const HP = stats.HP;
      const HPincr = 4 * (1 + 体力_乗算 / 100) * (1 + HP_乗算 / 100) / HP;
      return Math.floor(currentDamage * HPincr * hpValue);
    }
    if (key === '体力_乗算') {
      ...
      const HP_乗算 = stats.HP_乗算;
      const HP = stats.HP;
      const HPincr = 体力 * 0.01 * 4 * (1 + HP_乗算 / 100) / HP;
      return Math.floor(currentDamage * HPincr * hpValue);
    }
...
function getEquipmentEntryNames(entry) {
    const names = [entry.equipment_name, entry.name, entry.display_name];
    if (Array.isArray(entry.filter_names)) names.push(...entry.filter_names);
    if (Array.isArray(entry.selectable_options)) names.push(...entry.selectable_options);
    return names
        .filter(Boolean)
...
function shouldAlwaysIncludeEquipment(entry) {
    const names = getEquipmentEntryNames(entry);
    return names.some(name =>
        装備常時表示キーワード.some(keyword => name.includes(keyword))
    );
}

function buildEquipmentOptionsByCategory(entries) {
    const byCategory = { 強化: new Set(), エンチャ: new Set(), 覚醒: new Set() };

    for (const item of entries) {
        const category = item.category;
        if (!(category in byCategory)) continue;
        const selectable = Array.isArray(item.selectable_options) && item.selectable_options.length > 0
            ? item.selectable_options
            : (
                Array.isArray(item.filter_names) && item.filter_names.length > 0
                    ? item.filter_names
                    : [item.equipment_name]
            );
        for (const name of selectable) {
            if (name) byCategory[category].add(name);
        }
    }

    return Object.fromEntries(
        Object.entries(byCategory).map(([category, values]) => [category, [...values].sort()])
    );
}

// エンチャントデータ読み込み
function loadEnchantData() {
    try {
        // ENCHANT_DATAは既にグローバルスコープに定義済み
        if (typeof ENCHANT_DATA === 'undefined') {
            console.warn('ENCHANT_DATA not found, skipping enchant data load');
            return true;
        }

        // v2: 統合データ（強化/エンチャ/覚醒）
        if (Array.isArray(ENCHANT_DATA.entries)) {
            const parsedEntries = ENCHANT_DATA.entries.map(entry => ({
                id: entry.id,
                name: entry.equipment_name || entry.name || entry.display_name,
                equipment_name: entry.equipment_name || entry.name,
                category: entry.category || '不明',
                level: entry.level || 0,
                level_label: entry.level_label || '',
                detail: entry.detail || '',
                option_type: entry.option_type || 0,
                option_class: entry.option_class || 0,
                expected_values: entry.expected_values || {},
                base_expected_values: entry.base_expected_values || {},
                display_ranges: entry.display_ranges || {},
                filter_names: Array.isArray(entry.filter_names) ? entry.filter_names : [],
                selectable_options: Array.isArray(entry.selectable_options) ? entry.selectable_options : [],
                damage: 0
            }));

            // ゲーム更新時は ENCHANT_DATA.min_level（または latest_ui_min_level）を更新して調整する
            const latestMinLevel = Number(ENCHANT_DATA.latest_ui_min_level ?? ENCHANT_DATA.min_level ?? 0);

            エンチャント初期化データ = parsedEntries.filter(entry => {
                if (Number(entry.level || 0) >= latestMinLevel) return true;
                return shouldAlwaysIncludeEquipment(entry);
            });

            装備ランキング装備候補カテゴリ別.value = buildEquipmentOptionsByCategory(エンチャント初期化データ);

            装備ランキング装備候補.value = [...new Set(
                Object.values(装備ランキング装備候補カテゴリ別.value)
        ...
        // v1互換: 旧ポテンシャル期待値形式
        if (Array.isArray(ENCHANT_DATA.equipment)) {
            エンチャント初期化データ = ENCHANT_DATA.equipment.map(equip => ({
                id: equip.id,
                name: equip.name,
                ...
                level: equip.level,
                detail: '',
                expected_values: equip.expected_values,
                damage: 0
            }));
            装備ランキング装備候補カテゴリ別.value = {};
            装備ランキング装備候補.value = [...new Set(エンチャント初期化データ.map(item => item.equipment_name))].sort();
            return true;
        }

        console.warn('ENCHANT_DATA format is unsupported, skipping enchant data load');
        エンチャント初期化データ = [];
        装備ランキング装備候補カテゴリ別.value = {};
        装備ランキング装備候補.value = [];
        ...
        console.error('Failed to load enchant data:', error);
        return false;
    }
}
...
// 装備ランキング専用: 最終系を加算系へ寄せる
function normalizeEquipmentStatusType(statusType) {
  if (statusType === 'クリダメ_乗算') return 'クリダメ';
  if (statusType === '最大_乗算') return '最大';
  if (statusType === '最小_乗算') return '最小';
  return statusType;
}

function normalizeEquipmentExpectedValues(expectedValues) {
  const normalized = {};
  for (const [rawKey, rawValue] of Object.entries(expectedValues || {})) {
    const value = Number(rawValue || 0);
    if (!value) continue;
    const key = normalizeEquipmentStatusType(rawKey);
    normalized[key] = (normalized[key] || 0) + value;
  }
  return normalized;
}

function normalizeEquipmentDisplayRanges(displayRanges) {
  const normalized = {};
  for (const [rawKey, rangeText] of Object.entries(displayRanges || {})) {
    const key = normalizeEquipmentStatusType(rawKey);
    normalized[key] = rangeText;
  }
  return normalized;
}
...
/**
 * 覚醒ランキング計算の共通処理
 * @param {number} page - 覚醒ページ番号（1 or 2）
 */
function calculateAwakenRankings(page) {
  const emptyResult = { headers: ['順位', '名前', 'ステータス', 'ダメ換算'], data: [], totalDamage: 0 };
  
  const pageKey = `awaken${page}`;
  if (typeof awakenData === 'undefined' || !awakenData?.[pageKey]) return emptyResult;
  
  const points = (page === 1 ? 覚醒1ポイント : 覚醒2ポイント).value;
  if (!points || points <= 0) return emptyResult;
  
  const routes = generatePossibleRoutes(awakenData[pageKey].nodes, points);
  if (routes.length === 0) return emptyResult;
  
  const rankedRoutes = routes.map(route => ({
    damage: calculateRouteDamageValue(route),
    nodes: route
  })).sort((a, b) => b.damage - a.damage);
  
  const bestRoute = rankedRoutes[0];
  if (!bestRoute) return emptyResult;
  ...
  const data = bestRoute.nodes.map((node, index) => {
    ...
    node.stats.forEach(stat => {
      ...
      nodeDamage += calculateDamageIncrease(stat.name, stat.value);
    });
    
    totalDamage += nodeDamage;
    
    return {
      id: `awaken${page}_${index}`,
      name: node.name,
      stats: stats.join(' '),
      damage: nodeDamage
    };
  ...
  return { headers: ['順位', '名前', 'ステータス', 'ダメ換算'], data, totalDamage };
}

const calculateAwaken1Rankings = () => calculateAwakenRankings(1);
const calculateAwaken2Rankings = () => calculateAwakenRankings(2);

/**
 * 装備ランキング計算（カテゴリ別）
 * @param {string} targetCategory - 強化 / エンチャ / 覚醒
 */
function getEquipmentSelectableOptions(item) {
  if (Array.isArray(item.selectable_options) && item.selectable_options.length > 0) {
    return item.selectable_options;
  }
  if (Array.isArray(item.filter_names) && item.filter_names.length > 0) {
    return item.filter_names;
  }
  return [item.equipment_name || item.name].filter(Boolean);
}

function mergeExpectedValues(...expectedSets) {
  const merged = {};
...
function convertEquipmentDamage(totalDamage, damagePerStat) {
  if (換算方法.value === 'クリダメ換算') {
    const criDamageKey = inputs.value.攻撃タイプ === '物理' ? '物理_クリダメ' : '魔法_クリダメ';
    const criDamageValue = damagePerStat[criDamageKey];
    if (criDamageValue) return totalDamage / criDamageValue;
  } else if (換算方法.value === '筋魔換算') {
    const strIntKey = inputs.value.攻撃タイプ === '物理' ? '筋力' : '魔力';
    const strIntValue = damagePerStat[strIntKey];
    if (strIntValue) return totalDamage / strIntValue;
  }
  return totalDamage;
}

function buildUpgradeStateItems(sourceData) {
  const grouped = new Map();
  for (const item of sourceData) {
    const selectableOptions = getEquipmentSelectableOptions(item);
    const baseName = selectableOptions[0] || item.equipment_name || item.name;
    ...
    if (!grouped.has(baseName)) {
      grouped.set(baseName, {
        baseName,
        baseExpectedValues: {},
        maxUpgradeEntry: null,
        transcendEntry: null
      });
    }
    const group = grouped.get(baseName);
    const candidateBaseValues = item.base_expected_values || {};
    if (Object.keys(candidateBaseValues).length > Object.keys(group.baseExpectedValues).length) {
      group.baseExpectedValues = candidateBaseValues;
    }
    const detail = String(item.detail || '');
    if (detail.includes('超越')) {
      group.transcendEntry = item;
      continue;
    }
    if (!group.maxUpgradeEntry || Number(item.level || 0) >= Number(group.maxUpgradeEntry.level || 0)) {
      group.maxUpgradeEntry = item;
    }
  }

  const stateItems = [];
  for (const group of grouped.values()) {
    const baseEntry = group.maxUpgradeEntry || group.transcendEntry;
    if (!baseEntry) continue;

    const makeStateItem = (suffix, expectedValues, displayRanges, rawItem, stateOrder) => ({
      id: `${rawItem.id || group.baseName}-${suffix}`,
      name: `${group.baseName}（${suffix}）`,
      equipment_name: group.baseName,
      category: '強化',
      level: rawItem.level || 0,
      level_label: '',
      detail: suffix,
      expected_values: expectedValues,
      display_ranges: displayRanges || {},
      selectable_options: [`${group.baseName}（${suffix}）`],
      comparison_key: `${group.baseName}（${suffix}）`,
      state_order: stateOrder
    });

    const baseExpected = group.baseExpectedValues || {};
    stateItems.push(makeStateItem('未強化', baseExpected, {}, baseEntry, 0));

    const maxExpected = mergeExpectedValues(baseExpected, group.maxUpgradeEntry?.expected_values);
    const maxRanges = group.maxUpgradeEntry?.display_ranges || {};
    if (group.maxUpgradeEntry) {
      stateItems.push(makeStateItem('最大強化', maxExpected, maxRanges, group.maxUpgradeEntry, 1));
    }

    if (group.transcendEntry) {
      const transcendExpected = mergeExpectedValues(
        baseExpected,
        group.transcendEntry.expected_values
      );
      stateItems.push(
        makeStateItem('超越後', transcendExpected, group.transcendEntry.display_ranges || {}, group.transcendEntry, 2)
      );
    }
  }
...
function buildEquipmentRankingRow(item, includeLevel, damagePerStat) {
  let totalDamage = 0;
  const effectParts = [];

  const normalizedExpected = normalizeEquipmentExpectedValues(item.expected_values || {});
  const normalizedRanges = normalizeEquipmentDisplayRanges(item.display_ranges || {});
  for (const [statusType, value] of Object.entries(normalizedExpected)) {
    if (value && value > 0) {
      const dmgIncrease = calculateDamageIncrease(statusType, value);
      totalDamage += dmgIncrease;

      const displayValue = value < 10 ? value.toFixed(1) : Math.round(value);
      const rangeText = normalizedRanges[statusType];
      effectParts.push({
        text: `${statusDisplayLabels[statusType] || statusType}+${rangeText || displayValue}`,
        damage: dmgIncrease
      });
    }
  }

  effectParts.sort((a, b) => b.damage - a.damage);
  const absoluteDamage = convertEquipmentDamage(totalDamage, damagePerStat);
  const allEffects = effectParts.map(part => part.text);

  const row = {
    id: item.id,
    name: item.name || item.equipment_name || '',
    effect: allEffects.slice(0, 4).join(', ') || '効果なし',
    fullEffect: allEffects.join(', ') || '効果なし',
    absoluteDamage,
    damage: absoluteDamage
  };

  if (item.comparison_key) {
    row.comparisonKey = item.comparison_key;
  }
  if (includeLevel) {
    row.level = item.level_label || '';
  }

  return row;
}

function calculateEquipmentCategoryRankings(targetCategory) {
  const includeLevel = targetCategory === 'エンチャ';
  const headers = includeLevel
    ? ['順位', '装備名', 'ステータス', 'Lv', 'ダメージ']
    : ['順位', '装備名', 'ステータス', targetCategory === '強化' ? '差分ダメージ' : 'ダメージ'];

  if (!エンチャント初期化データ || エンチャント初期化データ.length === 0) {
    return { headers, data: [] };
  }
  
  const damagePerStat = results.value.damagePerStat;
  const selectedEquipment = (装備ランキング選択装備?.value || 装備ランキング未装備値).trim();
  
  let sourceData = エンチャント初期化データ.filter(item => item.category === targetCategory);

  if (targetCategory === '強化') {
    sourceData = buildUpgradeStateItems(sourceData);
  } else if (selectedEquipment && selectedEquipment !== 装備ランキング未装備値) {
    sourceData = sourceData.filter(item => getEquipmentSelectableOptions(item).includes(selectedEquipment));
  }
  ...
  const rankedData = sourceData.map(item => buildEquipmentRankingRow(item, includeLevel, damagePerStat));
  
  rankedData.sort((a, b) => b.absoluteDamage - a.absoluteDamage);

  if (targetCategory === '強化') {
    let baseDamage = 0;
    if (selectedEquipment && selectedEquipment !== 装備ランキング未装備値) {
      const baseRow = rankedData.find(item => item.comparisonKey === selectedEquipment);
      baseDamage = baseRow ? baseRow.absoluteDamage : 0;
    }
    rankedData.forEach(item => {
      item.damage = item.absoluteDamage - baseDamage;
    });
  }
...
const calculateEquipmentUpgradeRankings = () => calculateEquipmentCategoryRankings('強化');
const calculateEquipmentEnchantRankings = () => calculateEquipmentCategoryRankings('エンチャ');
const calculateEquipmentAwakenRankings = () => calculateEquipmentCategoryRankings('覚醒');
const calculateEnchantRankingsComputed = () => calculateEquipmentEnchantRankings();
...
class AwakenSkillTree {
  ...
  showTooltip(x, y, clientX, clientY, node) {
    ...
    html += `<span style="color: #999; font-size: 0.9em;">スロット${node.slot}</span><br>`;
    
    // エフェクト文字列を生成（rankings.jsのグローバル関数を使用）
    skillInfo.stats.forEach(stat => {
      const effectStr = buildEffectString(stat.name, stat.value);
      html += `${effectStr}<br>`;
    });
  ...
  // rankings.jsのグローバル関数buildEffectStringを使用
...
const updateSkillValues = (tabIndex = null) => {
  ...
  currentSkill.倍率 = skill.係数初期値 + (skill.係数上昇値 * level);
  currentSkill.追加ダメ = skill.固定ダメージ初期値 + (skill.固定ダメージ上昇値 * level);
...
const equipmentUpgradeRankings = computed(() => calculateEquipmentUpgradeRankings());
const equipmentEnchantRankings = computed(() => calculateEquipmentEnchantRankings());
const equipmentAwakenRankings = computed(() => calculateEquipmentAwakenRankings());

const 装備ランキング装備候補_現在タブ = computed(() => {
  const tabToCategory = {
    equipUpgrade: '強化',
    equipEnchant: 'エンチャ',
    equipAwaken: '覚醒'
  };
  const targetCategory = tabToCategory[activeMainTab.value];
  if (!targetCategory) {
    return 装備ランキング装備候補.value || [];
  }
  if (targetCategory === '強化') {
    return [...new Set(
      (equipmentUpgradeRankings.value.data || [])
        .map(item => item.comparisonKey)
        .filter(Boolean)
    )].sort((a, b) => a.localeCompare(b, 'ja'));
  }
  const optionsByCategory = 装備ランキング装備候補カテゴリ別.value || {};
  const options = optionsByCategory[targetCategory];
...
const 装備比較対象情報 = computed(() => {
  if (activeMainTab.value !== 'equipUpgrade') return null;
  const selected = (装備ランキング選択装備.value || '').trim();
  if (!selected || selected === 装備ランキング未装備値) return null;
  const target = equipmentUpgradeRankings.value.data.find(item => item.comparisonKey === selected);
  if (!target) return null;
  return {
    name: target.name,
    effect: target.fullEffect || target.effect || '効果なし',
    damage: target.absoluteDamage || 0
  };
});
...
const app = createApp({
  setup() {
    ...
    const autoOptimizeAwaken = async () => {
      覚醒ローディング中.value = true;
    ...
    const formatDamageValue = (damage) => {
      const safeDamage = Math.max(0, Math.floor(Number(damage) || 0));
      const oku = Math.floor(safeDamage / 100000000);
      const man = Math.floor((safeDamage % 100000000) / 10000);
      const remainder = Math.floor(safeDamage % 10000);
    ...
    const formatSignedDamageValue = (damage) => {
      const numeric = Number(damage) || 0;
      if (numeric === 0) return '0';
      const sign = numeric > 0 ? '+' : '-';
      return `${sign}${formatDamageValue(Math.abs(numeric))}`;
    };
    ...
    return {
      ...
      formatDamage: formatDamageValue,
      formatRankingDamage: (damage) => {
        const numeric = Number(damage) || 0;
        if (activeMainTab.value === 'equipUpgrade') {
          if (換算方法.value === '増加ダメ') {
            return formatSignedDamageValue(numeric);
          }
          return `${numeric > 0 ? '+' : ''}${numeric.toFixed(4)}`;
        }
        if (activeMainTab.value === 'guild' || activeMainTab.value.startsWith('awaken') || 換算方法.value !== '増加ダメ') {
          return numeric.toFixed(4);
        }
        return formatDamageValue(numeric);
      },
      ...
      onTabChange: (tabId) => {
        ...
        if (['equipUpgrade', 'equipEnchant', 'equipAwaken'].includes(tabId)) {
          const validEquipments = 装備ランキング装備候補_現在タブ.value;
          const defaultSelection = tabId === 'equipUpgrade' ? 装備ランキング未装備値 : '';
          if (!validEquipments.includes(装備ランキング選択装備.value)) {
            装備ランキング選択装備.value = defaultSelection;
          }
          if (tabId === 'equipUpgrade' && !装備ランキング選択装備.value) {
            装備ランキング選択装備.value = 装備ランキング未装備値;
          }
        }
      ...
      activeRanking: computed(() => {
        const rankings = {
          ...
          equipUpgrade: equipmentUpgradeRankings,
          equipEnchant: equipmentEnchantRankings,
          equipAwaken: equipmentAwakenRankings
        };
        return rankings[activeMainTab.value]?.value || { headers: [], data: [] };
      }),
      triggerCalculation: () => { },
      onJobChange,
      onSkillChange,
