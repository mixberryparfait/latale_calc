...
const getCurrentSkillData = () => {
  const skillIndex = parseInt(スキル名.value);
  if (!isNaN(skillIndex) && スキルマスター.value[skillIndex]) {
    return スキルマスター.value[skillIndex];
  }
  return null;
};
...
const loadFromStorage = () => {
  ...
  try {
    ...
    console.log('[DEBUG] localStorage data:', saved ? 'found' : 'not found');
    if (saved) {
      const data = JSON.parse(saved);
      console.log('[DEBUG] parsed data:', data);
      
      // inputs に直接代入
      console.log('[DEBUG] Before update - inputs.value.stats.筋力:', inputs.value.stats.筋力);
      inputs.value = data.inputs;
      console.log('[DEBUG] After update - inputs.value.stats.筋力:', inputs.value.stats.筋力);
      ...
      console.log('[DEBUG] 職業:', 職業.value, 'スキル名:', スキル名.value);
      return true;
    }
  } catch (error) {
    console.error('[loadFromStorage] Failed to load settings:', error);
  }
  return false;
};
...
const calculateRankings = skills => {
  ...
  for (const skill of skills) {
    ...
    for (let level = 1; level <= maxIterations; level++) {
      ...
      if ((skill.status_type1 === 15 && skill.status_type2 === 23) || 
...
const results = computed(() => {
  ...
  console.log('[DEBUG] Results calculated, saving to localStorage');
  saveToStorage();
...
const app = createApp({
  setup() {
    ...
    if (restored) {
      console.log('[DEBUG] 保存データを復元しました');
