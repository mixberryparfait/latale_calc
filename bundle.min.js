...
const updateEnemyValues = (enemyType) => {
  ...
  refs.敵ステータス.value.防御 = selectedEnemy.baseStats.defense;
  refs.敵ステータス.value.幸運 = selectedEnemy.baseStats.luck;

  if (refs.難易度.value) {
    const difficultyData = selectedEnemy.difficultyStats[refs.難易度.value];
    if (difficultyData) {
      refs.敵ステータス.value.ダメ減 = difficultyData.dmgReduce;
      refs.敵ステータス.value.ダメ減_乗算 = difficultyData.dmgReduceRatio;
      refs.敵ステータス.value.クリ減 = difficultyData.critReduce;
    }
  }
};
...
const results = computed(() => {
  ...
  for (const key of allKeys) {
    const generalValue = generalStats[key] || 0;
    const bossValue = bossStats[key] || 0;
    damagePerStat[key] = generalValue * generalImportance + bossValue * bossImportance;
  }
...
const 現在スキル単発ダメージ = computed(() => {
  console.log('[現在スキル単発ダメージ] 再計算開始');
  console.log('[現在スキル単発ダメージ] 選択中スキルインデックスの存在確認:', typeof 選択中スキルインデックス);
  
  // 選択中スキルインデックスへの明示的な依存を追加
  const index = 選択中スキルインデックス.value;
  console.log('[現在スキル単発ダメージ] index:', index);
  
  const skill = スキルリスト.value[index];
  console.log('[現在スキル単発ダメージ] skill:', skill);
  
  // スキルが存在しない場合はデフォルト値を返す
  if (!skill) {
    console.log('[現在スキル単発ダメージ] スキルが未定義のため、デフォルト値を返す');

  }
  
  const currentEnemy = 敵タブ.value === '一般' ? 敵ステータス_一般.value : 敵ステータス_ボス.value;
  console.log('[現在スキル単発ダメージ] currentEnemy:', currentEnemy);
  
  // スキルを引数として渡すことで、reactivityを確保
  const result = calculateCurrentSkillDamage(currentEnemy, 敵タブ.value, skill);
  console.log('[現在スキル単発ダメージ] result:', result);
...
const app = createApp({
  setup() {
    ...
    // デバッグ用：選択中スキルインデックスの変更を監視
    console.log('[DEBUG] 選択中スキルインデックスの初期値:', 選択中スキルインデックス.value);
    
    // watchをVue.watchではなくcomposition APIのwatchで試す
    const { watch } = Vue;
    watch(選択中スキルインデックス, (newVal, oldVal) => {
      console.log('[DEBUG] 選択中スキルインデックス変更:', oldVal, '->', newVal);
      console.log('[DEBUG] 現在のスキル:', スキルリスト.value[newVal]);
    });
    ...
    return {
      ...
      results: computed(() => {
        console.log('[results computed] 再計算開始');
        console.log('[results computed] 選択中スキルインデックス:', 選択中スキルインデックス.value);
        
        // 現在の敵タブでの選択スキルの単発ダメージを4つのカードに表示
        const singleSkillDamage = 現在スキル単発ダメージ.value;
        console.log('[results computed] singleSkillDamage:', singleSkillDamage);
        
        // 現在選択中のスキルのHIT数を取得
        const currentSkill = get現在スキル();
        const hitCount = currentSkill?.HIT数 || 1;
      ...
      incr: (stat) => {
        // 一般とボスの加重平均を計算
        const generalStat = 一般ダメージ.value?.damagePerStat?.[stat] || 0;
        const bossStat = ボスダメージ.value?.damagePerStat?.[stat] || 0;
        const bossImportance = ボス重要度.value / 100;
        const generalImportance = 1 - bossImportance;
        const baseStat = generalStat * generalImportance + bossStat * bossImportance;
        ...
        if (換算方法.value === '増加ダメ') {
          ...
          const criDamageKey = inputs.value.攻撃タイプ === '物理' ? '物理_クリダメ' : '魔法_クリダメ';
          const generalCriDamage = 一般ダメージ.value?.damagePerStat?.[criDamageKey] || 0;
          const bossCriDamage = ボスダメージ.value?.damagePerStat?.[criDamageKey] || 0;
          const criDamageValue = generalCriDamage * generalImportance + bossCriDamage * bossImportance;
          if (!criDamageValue) return baseStat;
          const ratio = baseStat / criDamageValue;
          // 1以上はそのまま、1未満は逆数で表示（+記号なし）
          return ratio >= 1 ? ratio.toFixed(2) : `1/${(1/ratio).toFixed(2)}`;
        } else if (換算方法.value === '筋魔換算') {
          const strIntKey = inputs.value.攻撃タイプ === '物理' ? '筋力' : '魔力';
          const generalStrInt = 一般ダメージ.value?.damagePerStat?.[strIntKey] || 0;
          const bossStrInt = ボスダメージ.value?.damagePerStat?.[strIntKey] || 0;
          const strIntValue = generalStrInt * generalImportance + bossStrInt * bossImportance;
